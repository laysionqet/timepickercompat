...
package moe.laysionqet.timepickercompat.time;
...
import moe.laysionqet.timepickercompat.HapticFeedbackController;
import moe.laysionqet.timepickercompat.R;
import moe.laysionqet.timepickercompat.Utils;
import moe.laysionqet.timepickercompat.time.RadialPickerLayout.OnValueSelectedListener;
...
public class TimePickerDialog extends DialogFragment implements OnValueSelectedListener {
    ...
    private boolean mHasUnselectedColor = false;

    private int mUnselectedColor;

    private boolean mHasPriamryColor = false;

    private int mPrimaryColor = 0;

    private boolean mHasPrimaryColorDark = false;

    private int mPrimaryColorDark = 0;
    ...
    public static class Builder {
        ...
        public Builder setOnTimeSetListener(OnTimeSetListener callback) {
            mTimePickerDialog.mCallback = callback;
            return this;
        }

        public Builder setHourOfDay(int hourOfDay) {
            mTimePickerDialog.mInitialHourOfDay = hourOfDay;
            return this;
        }

        public Builder setMinute(int minute) {
            mTimePickerDialog.mInitialMinute = minute;
            return this;
        }

        public Builder setIs24HourMode(boolean is24HourMode) {
            mTimePickerDialog.mIs24HourMode = is24HourMode;
            return this;
        }

        public Builder setPrimaryColor(int primaryColor) {
            mTimePickerDialog.mPrimaryColor = primaryColor;
            mTimePickerDialog.mHasPriamryColor = true;
            return this;
        }

        public Builder setPrimaryColorDark(int primaryColorDark) {
            mTimePickerDialog.mPrimaryColorDark = primaryColorDark;
            mTimePickerDialog.mHasPrimaryColorDark = true;
            return this;
        }

        public Builder setSelectedColor(int selectedColor) {
            mTimePickerDialog.mSelectedColor = selectedColor;
            mTimePickerDialog.mHasSelectedColor = true;
            return this;
        }

        public Builder setUnselectedColor(int unselectedColor) {
            mTimePickerDialog.mUnselectedColor = unselectedColor;
            mTimePickerDialog.mHasUnselectedColor = true;
            return this;
        }

        public Builder setThemeDark(boolean dark) {
            mTimePickerDialog.mThemeDark = dark;
            return this;
        }
    ...
    public static TimePickerDialog newInstance(OnTimeSetListener callback, int hourOfDay,
            int minute, boolean is24HourMode, int primaryColor, int primaryColorDark) {
        TimePickerDialog ret = new TimePickerDialog();
        ret.initialize(callback, hourOfDay, minute, is24HourMode, primaryColor, primaryColorDark);
        return ret;
    }
    ...
    public void initialize(OnTimeSetListener callback, int hourOfDay, int minute,
            boolean is24HourMode, int primaryColor, int primaryColorDark) {
        mCallback = callback;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (!mHasSelectedColor) {
            mSelectedColor = res.getColor(mThemeDark ? R.color.red : R.color.blue);
        }

        if (!mHasUnselectedColor) {
            mUnselectedColor = res
                    .getColor(mThemeDark ? R.color.white : R.color.numbers_text_color);
        }
        ...
        mPositiveButton = (TextView) view.findViewById(android.R.id.button3);
        ...
        mPositiveButton.setOnKeyListener(keyboardListener);

        mNegativeButton = (TextView) view.findViewById(android.R.id.button1);
        ...
        final int lightWhite = res.getColor(R.color.light_white);
        int timeDisplay = res.getColor(R.color.numbers_text_color);
        ColorStateList buttonTextColor = res.getColorStateList(R.color.done_text_color);

        int darkGray = res.getColor(R.color.dark_gray);
        int lightGray = res.getColor(R.color.light_gray);
        ColorStateList darkButtonTextColor = res.getColorStateList(R.color.done_text_color_dark);

        // Set the colors for each view based on the theme.
        view.setBackgroundColor(mThemeDark ? lightGray : lightWhite);
        ...
        if (mHasPriamryColor) {
            // 80% alpha
            final int dispBackgroundColor = Color.argb(0xcc, Color.red(mPrimaryColor),
                    Color.green(mPrimaryColor), Color.blue(mPrimaryColor));
            view.findViewById(R.id.time_display_background).setBackgroundColor(dispBackgroundColor);
        } else {
            view.findViewById(R.id.time_display_background).setBackgroundColor(
                    mThemeDark ? darkGray : lightWhite);
        }

        if (mHasPrimaryColorDark) {
            mPositiveButton.setTextColor(mPrimaryColorDark);
            mNegativeButton.setTextColor(mPrimaryColorDark);
        } else {
            mPositiveButton.setTextColor(mThemeDark ? darkButtonTextColor : buttonTextColor);
            mNegativeButton.setTextColor(mThemeDark ? darkButtonTextColor : buttonTextColor);
        }

        if (mHasUnselectedColor && mHasPrimaryColorDark) {
            mTimePicker.setThemeColor(mPrimaryColor, mPrimaryColorDark);
        }
     ...
     * 
     * @param keyCode The key to use as the first press. Keyboard mode will not
     *            be started if the key is not legal to start with. Or, pass in
     *            -1 to get into keyboard mode without a starting key.
     */
     ...
     * TimePicker's time.
     * 
     * @param updateDisplays If true, update the displays with the relevant
     *            time.
     */
     ...
     * 
     * @param allowEmptyDisplay if true, then if the typedTimes is empty, use
     *            the placeholder text. Otherwise, revert to the timepicker's
     *            values.
     */
    private void updateDisplay(boolean allowEmptyDisplay) {
        if (!allowEmptyDisplay && mTypedTimes.isEmpty()) {
            ...
            String hourStr = (values[0] == -1) ? mDoublePlaceholderText : String.format(hourFormat,
                    values[0]).replace(' ', mPlaceholderText);
     ...
     * minutes typed.
     * 
     * @param enteredZeros A size-2 boolean array, which the caller should
     *            initialize, and which may then be used for the caller to know
     *            whether zeros had been explicitly entered as either hours of
     *            minutes. This is helpful for deciding whether to show the
     *            dashes, or actual 0's.
    ...
    private int getAmOrPmKeyCode(int amOrPm) {
        ...
        if (mAmKeyCode == -1 || mPmKeyCode == -1) {
            ...
            for (int i = 0; i < Math.max(mAmText.length(), mPmText.length()); i++) {
                ...
                if (amChar != pmChar) {
                    KeyEvent[] events = kcm.getEvents(new char[] {
                            amChar, pmChar
                    });
